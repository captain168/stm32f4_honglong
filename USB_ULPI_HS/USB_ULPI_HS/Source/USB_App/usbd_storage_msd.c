...
const int8_t  STORAGE_Inquirydata[] = {//36
  ...
  0x00,	
  0x00,
  'W', 'B', '-', 'F', '4', ' ', ' ', ' ', /* Manufacturer : 8 bytes */
  'S', 'T', 'M', '3', '2', 'F', '4', ' ', /* Product      : 16 Bytes */
  'F', 'e', '.', 'M', ' ', ' ', ' ', ' ',
...
{
// 	uint32_t i;
//	printf("Read Blk_addr %x Blk_len %x \n\r",blk_addr,blk_len);
// 	if(blk_addr>=0x1e)          //读到了 数据交换区域
// 	{
// 	 DataReadSign=1;
...
//  for(i=0;i<blk_len*BLOCK_SIZE;i++)
//  {
// 	 	 buf[i]=Memory[blk_addr*BLOCK_SIZE+i];
//  }
  return 0;
}
/**
...
{
    uint32_t i;
//printf("Write Blk_addr %x Blk_len %x \n\r",blk_addr,blk_len);	
 for(i=0;i<blk_len*BLOCK_SIZE;i++)
 {
	 U_DISK[blk_addr*BLOCK_SIZE+i]=buf[i];
 }
  return (0);
}
